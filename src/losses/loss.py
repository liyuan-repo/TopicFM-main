...
class TopicFMLoss(nn.Module):
    ...
    def compute_coarse_loss(self, data, match_ids=None, weight=None):
        ...
        Args:
            data (torch.Tensor): (N, HW0, HW1) / (N, HW0+1, HW1+1)
            match_ids (torch.Tensor): (N, HW0, HW1)
        ...
        if "geo_conf_pairs" in data:
            ...
            loss_geo = F.binary_cross_entropy_with_logits(geo_conf_pairs[valid_pairs], geo_labels[valid_pairs],
